  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'myPlants': {
        ...
        if (!activePlants.length) return renderEmptyState('eco', 'You don\'t have any active listings', 'Add a Plant', () => navigation.navigate('AddPlant'));
        return renderPlantList(activePlants);
      }
      case 'favorites': {
        if (!user.favorites?.length) return renderEmptyState('favorite-border', 'You don\'t have any saved plants', 'Browse Plants', handleBrowsePlants);
        return renderPlantList(user.favorites);
      }
      case 'sold': {
        const soldPlants = user.listings?.filter(plant => plant.status === 'sold') || [];
        if (!soldPlants.length) return renderEmptyState('local-offer', 'You haven\'t sold any plants yet');
        return renderPlantList(soldPlants);
      }
      default:
        return null;
    }
...
/>
      ...
      <View style={styles.statsRow}>
        ...
        <View style={styles.statBox}>
          <Text style={styles.statValue}>{user.stats?.rating?.toFixed?.(1) || '0.0'}</Text>
          <Text style={styles.statLabel}>Rating</Text>
        </View>
      </View>

      <View style={styles.tabsContainer}>
        {['myPlants', 'favorites', 'sold'].map(tab => (
          <TouchableOpacity
            key={tab}
          ...
          >
            <MaterialIcons
              name={tab === 'myPlants' ? 'eco' : tab === 'favorites' ? 'favorite' : 'local-offer'}
              size={24}
              color={activeTab === tab ? '#4CAF50' : '#666'}
            />
            <Text style={[styles.tabText, activeTab === tab && styles.activeTabText]}>
              {tab === 'myPlants' ? 'My Plants' : tab.charAt(0).toUpperCase() + tab.slice(1)}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
...
const styles = StyleSheet.create({
  ...
  profileCard: {
    backgroundColor: '#fff', margin: 16, padding: 20, borderRadius: 16, alignItems: 'center',
  ...
  actionButton: {
    marginTop: 16, backgroundColor: '#4CAF50', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 6,
  },
  actionButtonText: { color: '#fff', fontWeight: '600' },
  addPlantButton: {
    position: 'absolute', bottom: 16, right: 16, backgroundColor: '#4CAF50',
    borderRadius: 30, padding: 16, elevation: 4,
