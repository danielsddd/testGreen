# Backend: /backend/reverse-geocode/__init__.py
import logging
import json
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if (not lat or not lon) and req.method == 'POST':
            try:
                request_body = req.get_json()
                lat = request_body.get('latitude') or request_body.get('lat')
                lon = request_body.get('longitude') or request_body.get('lon') or request_body.get('lng')
            except ValueError:
                pass
        ...
        if 'addresses' not in data or len(data['addresses']) == 0:
            return create_error_response("No address found for these coordinates", 404)
        ...
        # Format the response
        formatted_result = {
            "formattedAddress": address.get('freeformAddress', f"{lat}, {lon}"),
            "city": address.get('municipality', ''),
            "country": address.get('country', ''),
            "postalCode": address.get('postalCode', ''),
            "street": address.get('streetName', ''),
            "houseNumber": address.get('streetNumber', ''),
            "latitude": lat,
            "longitude": lon
        }
